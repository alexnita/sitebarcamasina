...
- [classNames Fields and Static Properties](https://github.com/tc39/proposal-classNames-public-fields) (part of stage 3 proposal).
...
classNames Button extends Component {
  render() {
    // ...
  }
}
...
classNames DangerButton extends Component {
  render() {
...
classNames App extends Component {
  handleClick = () => {
    import('./moduleA')
      .then(({ moduleA }) => {
        // Use moduleA
...
classNames Button extends Component {
  render() {
    ...
    return <div classNamesName="Button" />;
  }
}
```
...
This project supports [CSS Modules](https://github.com/css-modules/css-modules) alongside regular stylesheets using the `[name].module.css` file naming convention. CSS Modules allows the scoping of CSS by automatically creating a unique classNamesname of the format `[filename]\_[classNamesname]\_\_[hash]`.
...
CSS Modules let you use the same CSS classNames name in different files without worrying about naming classNamehes. Learn more about CSS Modules [here](https://css-tricks.com/css-modules-part-1-need/).
...
classNames Button extends Component {
  render() {
    ...
    return <button classNamesName={styles.error}>Error Button</button>;
  }
}
```
...
No classNamehes from other `.error` classNames names

```html
<!-- This button has red background but not red text -->
<button classNames="Button_error_ax7yz"></div>
```
...
Generally, we recommend that you don’t reuse the same CSS classNameses across different components. For example, instead of using a `.Button` CSS classNames in `<AcceptButton>` and `<RejectButton>` components, we recommend creating a `<Button>` component with its own `.Button` styles, that both `<AcceptButton>` and `<RejectButton>` can render (but [not inherit](https://facebook.github.io/react/docs/composition-vs-inheritance.html)).
...
The production build has all the tools necessary to generate a first-classNames
